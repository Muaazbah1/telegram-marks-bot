...
def parse_pdf_marks(pdf_path):
    ...
    **ملاحظة هامة:** هذه الدالة تحتاج إلى تعديل دقيق بناءً على شكل ملف PDF الفعلي.
    ...
    # مثال: البحث عن 9 أرقام متبوعة بمسافة ثم رقم (العلامة)
    # regex = r"(\d{9})\s+([\d\.]+)"
    ...
    for page in doc:
        ...
        for line in lines:
            ...
            # هذا التعبير يحاول العثور على 9 أرقام (الرقم الجامعي) متبوعة بأي شيء، ثم رقم عشري أو صحيح (العلامة)
            match = re.search(r"(\d{9}).*?([\d\.]+)", line)
